...
(function () {
  ...
  const CHAPTERS = [
    ['#top', '01 · Plataforma'],
    ['#problem', '02 · Principio'],
    ['#method', '03 · Método'],
    ['#modules', '04 · Módulos'],
    ['#platform', '05 · La plataforma'],
    ['#casestudy', '06 · En operación'],
    ['#sectors', '07 · Sectores'],
    ['.diff-section', '08 · Diferenciadores'],
    ['#security', '09 · Seguridad'],
    ['#implementation', '10 · Implementación'],
    ['.founder-section', '11 · Promesa'],
    ['#demo', '12 · Contacto'],
  ];
  ...
  function buildAltimeter() {
    ...
    el.innerHTML =
      '<div class="alt-scale"><span class="alt-needle"></span></div>' +
      '<div class="alt-read"><span class="alt-value">0000</span><span class="alt-unit">m.s.n.m.</span></div>' +
      '<div class="alt-chapter">01 · Plataforma</div>';
  ...
  function init() {
      ...
      ST.create({
        ...
        onUpdate(self) {
          gsap.to(state, {
            ...
            ease: 'power2.out',
            overwrite: true,
            onUpdate() { valueEl.textContent = String(Math.round(state.v)).padStart(4, '0'); },
          });
      ...
      CHAPTERS.forEach(([sel, label]) => {
        ...
        ST.create({
          ...
          onToggle(self) {
            ...
            gsap.fromTo(chapterEl, { opacity: 0, y: 6 }, { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out' });
          },
        });
      });
    }
    ...
    if (heroSection) {
      gsap.to('.hero-headline, .hero-subtitle, .hero-meta', {
        ...
        opacity: 0.15,
        ease: 'none',
        scrollTrigger: { trigger: heroSection, start: 'top top', end: 'bottom 35%', scrub: 0.6 },
      });
    ...
    document.querySelectorAll('main section h2').forEach((h) => {
      ...
      gsap.fromTo(h,
        ...
        {
          clipPath: 'inset(0 0 -8% 0)', y: 0, duration: 0.9, ease: 'power3.out',
      ...
      profile.querySelectorAll('polyline').forEach((line) => {
        ...
        gsap.fromTo(line,
          { strokeDasharray: len, strokeDashoffset: len },
          {
            ...
            scrollTrigger: { trigger: '#method', start: 'top 70%', end: 'center 45%', scrub: 0.5 },
          });
      });
      gsap.fromTo(profile.querySelectorAll('circle'),
        { scale: 0, transformOrigin: 'center' },
        {
          scale: 1, stagger: 0.18, duration: 0.4, ease: 'back.out(2.5)',
          scrollTrigger: { trigger: '#method', start: 'top 55%', once: true },
        });
    }
    ...
    if (methodCells.length) {
      gsap.fromTo(methodCells,
        { opacity: 0, y: 36 },
        {
          ...
          scrollTrigger: { trigger: '.method-grid', start: 'top 80%', once: true },
        });
    }

    // ---- 5. Parallax en cordilleras divisorias y eje de implementación ----
    window.gsap.utils.toArray('.cordillera-divider').forEach((svg) => {
      gsap.fromTo(svg, { y: 26 }, {
        y: -26, ease: 'none',
        scrollTrigger: { trigger: svg, start: 'top bottom', end: 'bottom top', scrub: 1 },
      });
    });
    ...
    if (axis) {
      ...
      const w = 1188;
      gsap.fromTo(line, { attr: { x2: 0 } }, {
        attr: { x2: w }, ease: 'none',
        scrollTrigger: { trigger: '#implementation', start: 'top 70%', end: 'center 50%', scrub: 0.5 },
      });
      gsap.fromTo(axis.querySelectorAll('circle'),
        { scale: 0, transformOrigin: 'center' },
        {
          scale: 1, stagger: 0.2, duration: 0.35, ease: 'back.out(2.5)',
          scrollTrigger: { trigger: '#implementation', start: 'top 55%', once: true },
        });
    }
    ...
    if (track) {
      ...
      ST.create({
        ...
        onUpdate(self) {
          ...
          gsap.to(speed, { factor: 1 + v, duration: 0.4, overwrite: true });
        },
      });
      gsap.ticker.add(() => {
  ...
  else window.addEventListener('load', () => setTimeout(init, 900));
})();
